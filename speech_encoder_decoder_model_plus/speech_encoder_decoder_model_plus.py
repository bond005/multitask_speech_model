...
class SpeechEncoderDecoderModel(PreTrainedModel):
    ...
    :meth*~transformers.AutoModelForCTC.from_pretrained* class method for the encoder and
    :meth*~transformers.AutoModelForCausalLM.from_pretrained* class method for the decoder.
    """
    config_class = SpeechEncoderDecoderConfig
    base_model_prefix = "speech_encoder_decoder_plus"
    ...
    def __init__(
        ...
        config.encoder.output_hidden_states = True
        ...
        **kwargs
        ...
        if encoder is None:
            ...
            if "config" not in kwargs_encoder:
                ...
                kwargs_encoder["config"].output_hidden_states = True

            encoder = AutoModelForCTC.from_pretrained(encoder_pretrained_model_name_or_path, *model_args, **kwargs_encoder)
        else:
            encoder.config.output_hidden_states = True
    ...
    def forward(
        ...
        **kwargs,
    ) -> Union[Tuple[torch.FloatTensor], MultitaskSeq2SeqLMOutput]:
        ...
        if labels is None:
            ...
            if return_dict:
                ...
                encoder_logits = encoder_outputs[0]
            ...
            if return_dict:
                ...
                encoder_logits = encoder_outputs[1]
        ...
        if labels is not None:
            if encoder_labels.max() >= self.config.vocab_size:
                raise ValueError(f"Label values must be <= vocab_size: {self.encoder.config.vocab_size}")

            logits = decoder_outputs.logits if return_dict else decoder_outputs[0]
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(logits.reshape(-1, self.decoder.config.vocab_size), labels.reshape(-1)) + encoder_loss
